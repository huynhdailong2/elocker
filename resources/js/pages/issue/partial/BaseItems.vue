...
export default {
  ...
  computed: {
    selectedSpares() {
      return chain(this.data)
        .filter(
          (item) => item.is_checked || includes(this.selectedItems, item.id)
        )
        .value();
    },
  ...
  methods: {
        ...
        .then((res) => {
          ...
          this.data = chain(data.spare_bins || [])
            ...
            .map((item, index) => {
              ...
              if (
                item.type === Const.ITEM_TYPE.EUC.value &&
                item.euc_box_order
              ) {
    ...
    getLoadHydrostaticDueDate(record) {
      const item = head(record.configures || []);
      return item && item.has_load_hydrostatic_test_due
