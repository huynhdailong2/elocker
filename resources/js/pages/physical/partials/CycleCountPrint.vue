  ...
  <div id="cycle-count-print"
    ...
    <div class="container mt-3">
      ...
      <h2 class="text-left mb-4">
        <span class="font-weight-bold">Cycle Count #:</span>
        <span>{{ uuid }}</span>
      </h2>
      </div>
      ...
      <div class="row">
        ...
        <div class="border border-dark p-2 text-break text-dark col-2" :class="{'col-3': config.withoutQty}">Description</div>
        <div class="border border-dark p-2 text-break text-center text-dark col-3">Location</div>
        <div class="border border-dark p-2 text-break text-center text-dark col-2">Item Type</div>
      ...
      <div class="row" v-for="(item, index) in data">
        ...
        <div class=" col-3 border border-dark p-2 text-break text-dark text-center">{{ item.cluster_name }} - {{ item.shelf_name }} - {{ item.row }} - {{ item.bin }}</div>
        <div class=" col-2 border border-dark p-2 text-break text-dark text-center text-capitalize">{{ item.type }}</div>
      ...
      <div class="row"  v-if="!data.length">
        <div class=" col-12 border border-dark p-2 text-center text-break text-dark">There is no data.</div>
      </div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      htmlToPaper (callback) {
        ...
        const element = document.getElementById('cycle-count-print');

        if(!element) {
          this.isPrinting = false;
          alert(`Element to print #cycle-count-print not found!`);
          return;
        }
        ...
        win.document.write(`
          <html>
            <head>
              <title>Cycle Count - ${this.current}</title>
            </head>
            <body>
              ${element.innerHTML}
            </body>
