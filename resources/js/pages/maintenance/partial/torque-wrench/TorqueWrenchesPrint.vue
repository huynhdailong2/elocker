  ...
  <div id="torque-wrench-print"
    ...
    <div class="container mt-3">
      ...
      <h2 class="text-center mb-4">
        <span class="font-weight-bold">Torque Wrench</span>
      </h2>
      </div>
      ...
      <div class="row" style="background-color: #b3c6e7;">
        ...
        <div class="border border-dark p-2 text-break text-center text-dark col-1">Torque Wrench No</div>
        ...
        <div class="border border-dark p-2 text-break text-center text-dark col-2">Expiry</div>
      </div>
      ...
      <div class="row" v-for="(item, index) in data">
        ...
        <div class=" col-2 border border-dark p-2 text-break text-center text-dark">{{ item.issued_date | dateFormatter('YYYY-MM-DD HH:mm:ss', 'DD-MM-YYYY') }}</div>
        ...
        <div class=" col-1 border border-dark p-2 text-break text-dark text-center">{{ item.torque_value | formatQuantity }}</div>
        ...
        <div class=" col-2 border border-dark p-2 text-break text-dark text-center">{{ item.expiry_date | dateFormatter('YYYY-MM-DD HH:mm:ss', 'DD-MM-YYYY') }}</div>
      </div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      htmlToPaper (callback) {
        ...
        const element = document.getElementById('torque-wrench-print');

        if(!element) {
          this.isPrinting = false;
          alert(`Element to print #torque-wrench-print not found!`);
          return;
        }
        ...
        win.document.write(`
          <html>
            <head>
              <title>Torque Wrench - ${this.current}</title>
            </head>
            <body>
              ${element.innerHTML}
            </body>
