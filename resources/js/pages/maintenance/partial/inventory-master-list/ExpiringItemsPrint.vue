<template>
  <div id="expiring-items-print"
    ...
    <div class="container" style="margin: 10px 10px 0px">
      <h2 class="mb-3">
        <span class="font-weight-bold">Expiring Items Count</span>
      </h2>

      <div class="mb-3" style="display: flex;justify-content: flex-end;">
        <button class="btn is-refresh" style="border: 1px solid black;border-radius: 0;padding: 10px 6px;background: #FFFFFF;">Refresh</button>
        <button class="btn is-expired box" style="border: 1px solid black;border-radius: 0;padding: 10px 6px;background: #f21501;">Expired / Less than 1 month</button>
        <button class="btn is-in-30-days box" style="border: 1px solid black;border-radius: 0;padding: 10px 6px;background: #f7bf03;">1 – 2 months to expired</button>
        <button class="btn is-in-60-days box" style="border: 1px solid black;border-radius: 0;padding: 10px 6px;background: #70ad47;">More than 2 months to expired</button>
      </div>

      <div >
        <table class="" style="width: 100%;">
          <tr>
            <th style="background-color: #b3c6e7; width: 100px;">Load/Hydrostatic Test Due</th>
            <th style="background-color: #b3c6e7; width: 100px;">Calibration Due</th>
            <th style="background-color: #b3c6e7; width: 100px;">Expiring Date</th>
            <th style="background-color: #b3c6e7;">Item Details</th>
            <th style="background-color: #b3c6e7; width: 90px;">Item Type</th>
            <th style="background-color: #b3c6e7; width: 70px;">Part No</th>
            <th style="background-color: #b3c6e7; width: 50px;">Quantity</th>
            <th style="background-color: #b3c6e7;">Item Location</th>
          </tr>
          <tr v-for="(item, index) in data" :style="{ 'background-color': colorSpare(item) }">
            <td class=" col-2 border border-dark p-2 text-break text-center text-dark">{{ item.load_hydrostatic_test_due | dateFormatter(Const.DATE_PATTERN, 'DD-MM-YYYY') }}</td>
            <td class=" col-2 border border-dark p-2 text-break text-center text-dark">{{ item.calibration_due | dateFormatter(Const.DATE_PATTERN, 'DD-MM-YYYY') }}</td>
            <td class=" col-2 border border-dark p-2 text-break text-center text-dark">{{ item.expiry_date | dateFormatter(Const.DATE_PATTERN, 'DD-MM-YYYY') }}</td>
