  ...
  <div id="expiring-items-print"
    ...
    <div class="container mt-3">
      ...
      <h2 class="text-center mb-4">
        <span class="font-weight-bold">Yet to Return Items Count</span>
      </h2>
      </div>
      ...
      <div class="row" style="background-color: #b3c6e7;">
        ...
        <div class="border border-dark p-2 text-break text-center text-dark col-1">Loan Date</div>
        <div class="border border-dark p-2 text-break text-center text-dark col-1">Yet to Return</div>
        ...
        <div class="border border-dark p-2 text-break text-center text-dark col-1">Loan By</div>
      </div>

      <!-- <div class="row" v-for="(item, index) in data" :style="{ 'background-color': item.expired_return_time ? '#f21501' : '' }"> -->
      <div class="row" v-for="(item, index) in data">
        <div class=" col-1 border border-dark p-2 text-break text-center text-dark">{{ item.load_hydrostatic_test_due | dateFormatter('YYYY-MM-DD HH:mm:ss', 'DD-MM-YYYY') }}</div>
        <div class=" col-1 border border-dark p-2 text-break text-center text-dark">{{ item.calibration_due | dateFormatter('YYYY-MM-DD HH:mm:ss', 'DD-MM-YYYY') }}</div>
        ...
        <div class=" col-1 border border-dark p-2 text-break text-dark text-center">{{ item.yet_to_return }}</div>
        ...
        <div class=" col-1 border border-dark p-2 text-break text-dark text-center">{{ item.issued_quantity }}</div>
    ...
    methods: {
      ...
      htmlToPaper (callback) {
        ...
        win.document.write(`
          <html>
            <head>
              <title>Yet To Return Items - ${this.current}</title>
            </head>
            <body>
              ${element.innerHTML}
            </body>
