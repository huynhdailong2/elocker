  ...
  <div class="table-scroller">
    <table>
      <thead>
        ...
        <th>Type</th>
        <th>Qty OH</th>
        <th v-if="isReplenishStep">Action</th>
      </thead>
      <tbody>
...
export default {
  ...
  computed: {
    ...
    noReplenishData () {
      return isEmpty(this.replenishData)
    },
  ...
  methods: {
    ...
    onClickCheckout () {
      if (this.noReplenishData) {
        return
      }

      const replenishPolCallback = (pols) => {
        rf.getRequest('AdminRequest').replenishPol({ pols })
          .then(res => {
            this.currentStep = STEP.CHOOSE_ITEMS
            this.showSuccess()
            this.$emit('done')
      ...
      const callback = () => {
        this.$modal.show(this.scanTakerModal, {
          type: Const.POL_ACTION.REPLENISH,
          data: this.replenishData,
          callback: replenishPolCallback
        })
      }
