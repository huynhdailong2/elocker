  ...
  <div id="replenish-pols-print"
    ...
    <div class="container mt-3">
      <div class="row text-center">
        <h2 class="mb-4">
          <span class="font-weight-bold">Replenished POL - {{ current }}</span>
        </h2>
      </div>
      ...
      <div class="row" style="background-color: #b3c6e7;">
        ...
        <div class="border border-dark p-2 text-break text-center text-dark col-1">Qty Replenish</div>
        <div class="border border-dark p-2 text-break text-center text-dark col-1">Handover By</div>
        <div class="border border-dark p-2 text-break text-center text-dark col-1">Receiver By</div>
      </div>
      ...
      <div class="row" v-for="(item, index) in data">
        ...
        <div class=" col-1 border border-dark p-2 text-break text-dark text-center">{{ item.requester_name }}</div>
        <div class=" col-1 border border-dark p-2 text-break text-dark text-center">{{ item.receiver_name }}</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    htmlToPaper (callback) {
      ...
      const element = document.getElementById('replenish-pols-print');

      if(!element) {
        this.isPrinting = false;
        alert(`Element to print #replenish-pols-print not found!`);
        return;
      }
      ...
      win.document.write(`
        <html>
          <head>
            <title>Replenished POLs - ${this.current}</title>
          </head>
          <body>
            ${element.innerHTML}
          </body>
